(function() {
    ...
    if (!token) {
      ...
      setTimeout(function() {
        ...
        message.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);' +
          'background:#222;border:2px solid #fff;padding:20px;border-radius:8px;z-index:9999;min-width:300px;';
        message.innerHTML = `
          <h3 style="color:#fff;margin-top:0;">Требуется токен Discogs</h3>
          <p style="color:#ccc;font-size:12px;">Получить токен: https://www.discogs.com/settings/developers</p>
          <input type="text" id="discogs-token-input" placeholder="Введите токен" style="width:100%;padding:8px;box-sizing:border-box;margin:10px 0;">
          <button id="save-token-btn" style="width:100%;padding:8px;background:#0066cc;color:#fff;border:none;border-radius:4px;cursor:pointer;">Сохранить</button>
          <button id="skip-token-btn" style="width:100%;padding:8px;margin-top:5px;background:#666;color:#fff;border:none;border-radius:4px;cursor:pointer;">Пропустить</button>
        `;
  ...
  function showMusicInterface() {
    ...
    let html = `
      <div style="margin-bottom:20px;">
        <h1 style="color:#fff;margin:0 0 15px 0;">Поиск Музыки 🎵</h1>
        
        <div style="display:flex;gap:10px;margin-bottom:20px;">
          <input 
            type="text" 
            id="music-search-input" 
            placeholder="Введите исполнителя или трек (напр: The Beatles Hey Jude)" 
            style="flex:1;padding:10px;border:1px solid #666;background:#1a1a1a;color:#fff;border-radius:4px;font-size:14px;"
          >
          <button 
            id="music-search-btn" 
            style="padding:10px 20px;background:#0066cc;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:14px;"
          >Поиск</button>
        </div>
        
        <div style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap;">
          <label style="display:flex;align-items:center;gap:5px;color:#ccc;font-size:12px;">
            <input type="checkbox" id="filter-seeders" checked>
            Фильтр по сидерам
          </label>
          <label style="display:flex;align-items:center;gap:5px;color:#ccc;font-size:12px;">
            <input type="checkbox" id="sort-seeders">
            Сортировать по сидерам
          </label>
          <button id="settings-btn" style="margin-left:auto;padding:5px 10px;background:#444;color:#fff;border:none;border-radius:4px;cursor:pointer;font-size:12px;">⚙️ Настройки</button>
        </div>
      </div>
      
      <div id="music-loading" style="color:#999;display:none;text-align:center;padding:20px;">
        Загрузка... ⏳
      </div>
      
      <div id="music-results" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:15px;"></div>
    `;
    ...
    document.getElementById('music-results').innerHTML = `
      <div style="grid-column:1/-1;color:#999;padding:40px;text-align:center;">
        <p style="font-size:16px;">👇 Введите название трека или исполнителя выше</p>
        <p style="font-size:12px;margin-top:10px;">Примеры: "Pink Floyd Comfortably Numb" или "Daft Punk - Get Lucky"</p>
      </div>
    `;
  }
  ...
  function searchMusic(query) {
    ...
    if (!parserUrl || parserUrl === 'http://localhost:9117') {
      ...
      results.innerHTML = `
        <div style="grid-column:1/-1;color:#f88;padding:20px;background:#1a0000;border-radius:4px;">
          <strong>❌ Парсер не настроен!</strong><br>
          <p style="font-size:12px;margin:5px 0;">В Lampa откройте: Настройки → Парсер → Выберите парсер и введите URL</p>
          <p style="font-size:12px;margin:5px 0;">Обычно: http://localhost:9117</p>
        </div>
      `;
      return;
    }
    ...
    let searchUrl = parserUrl + '/api/v2.0/indexers/all/results?' +
      'apikey=' + parserKey +
    ...
    fetch(searchUrl)
      ...
      .then(data => {
        ...
        if (!data.Results || data.Results.length === 0) {
          results.innerHTML = `
            <div style="grid-column:1/-1;color:#999;padding:20px;text-align:center;">
        ...
        if (musicTorrents.length === 0) {
          results.innerHTML = `
            <div style="grid-column:1/-1;color:#999;padding:20px;text-align:center;">
              Музыкальные торренты не найдены
            </div>
          `;
          return;
        }
        ...
        results.innerHTML = '<div style="grid-column:1/-1;color:#999;text-align:center;padding:20px;">Загрузка обложек...</div>';
      })
      .catch(error => {
        loading.style.display = 'none';
        results.innerHTML = `
          <div style="grid-column:1/-1;color:#f88;padding:20px;background:#1a0000;border-radius:4px;">
            <strong>❌ Ошибка подключения к парсеру</strong><br>
            <p style="font-size:12px;margin:5px 0;">Убедитесь, что парсер (Jackett) запущен на адресе: ${parserUrl}</p>
            <p style="font-size:12px;margin:5px 0;">Ошибка: ${error.message}</p>
          </div>
        `;
  ...
  function displayTorrentResult(torrent, cover, originalQuery) {
    ...
    if (resultsContainer.querySelector('[style*="No Cover"]')) {
      resultsContainer.innerHTML = '';
    }

    let resultElement = document.createElement('div');
    resultElement.style.cssText = 'background:#1a1a1a;border-radius:8px;overflow:hidden;cursor:pointer;' +
    ...
    resultElement.innerHTML = `
      <div style="width:100%;height:180px;background:#0a0a0a;overflow:hidden;">
        <img src="${cover}" alt="cover" style="width:100%;height:100%;object-fit:cover;background:#333;">
      </div>
      <div style="padding:10px;">
        <div style="color:#fff;font-size:12px;font-weight:bold;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:5px;">
          ${torrent.Title.substring(0, 30)}...
        </div>
        <div style="color:#999;font-size:10px;margin-bottom:8px;">
          👥 ${torrent.Seeders || 0} | 📁 ${formatSize(torrent.Size)}
        </div>
        <div style="display:flex;gap:5px;">
          <button class="play-torrent-btn" style="flex:1;padding:6px;background:#0066cc;color:#fff;border:none;border-radius:3px;cursor:pointer;font-size:11px;font-weight:bold;">
            ▶️ Играть
          </button>
          <button class="info-torrent-btn" style="flex:0;padding:6px 8px;background:#444;color:#fff;border:none;border-radius:3px;cursor:pointer;font-size:11px;">
            ℹ️
          </button>
        </div>
      </div>
...
Добавлен: ${new Date(torrent.PublishDate).toLocaleDateString('ru-RU')}
  ...
  function showSettings() {
    ...
    let settingsHtml = `
      <div style="max-width:600px;background:#1a1a1a;border:1px solid #444;border-radius:8px;padding:20px;">
        <h2 style="color:#fff;margin-top:0;">⚙️ Настройки плагина</h2>
        
        <div style="margin-bottom:20px;">
          <label style="display:block;color:#ccc;margin-bottom:5px;font-weight:bold;">Токен Discogs:</label>
          <input 
            type="password" 
            id="discogs-token-setting" 
            value="${discogsToken}" 
            placeholder="Получить на https://www.discogs.com/settings/developers"
            style="width:100%;padding:8px;box-sizing:border-box;background:#0a0a0a;border:1px solid #444;color:#fff;border-radius:4px;"
          >
          <p style="color:#999;font-size:11px;margin:5px 0;"><a href="https://www.discogs.com/settings/developers" style="color:#0066cc;text-decoration:none;">🔗 Получить токен →</a></p>
        </div>
        
        <div style="margin-bottom:20px;">
          <label style="display:block;color:#ccc;margin-bottom:5px;font-weight:bold;">URL TorrServer:</label>
          <input 
            type="text" 
            id="torrserver-url-setting" 
            value="${torrserverUrl}" 
            placeholder="http://127.0.0.1:8090"
            style="width:100%;padding:8px;box-sizing:border-box;background:#0a0a0a;border:1px solid #444;color:#fff;border-radius:4px;"
          >
          <p style="color:#999;font-size:11px;margin:5px 0;">Адрес вашего TorrServer сервера</p>
        </div>
        
        <div style="display:flex;gap:10px;">
          <button id="save-settings-btn" style="flex:1;padding:10px;background:#0066cc;color:#fff;border:none;border-radius:4px;cursor:pointer;font-weight:bold;">✓ Сохранить</button>
          <button id="back-settings-btn" style="flex:1;padding:10px;background:#444;color:#fff;border:none;border-radius:4px;cursor:pointer;">← Назад</button>
        </div>
      </div>
    `;
